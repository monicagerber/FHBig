...
- functions and methods to handle the different manipulations to performed on the objects
...
Discussion: classes and generic functions/methods
...
- explicitly defined classes and generic function
- multiple inheritance
- increased complexity of use

## S3: generic functions 
A generic function has a single expression which is the call to `UseMethod`.
```{r S3_method}
#' print() is a generic function
print
```
`UseMethod` is the mechanism that helps to dispatch to the appropriate method.
A generic function does dispatch through a call to `UseMethod` that initaites the dispatch one single argument, usually the first one.

## S3: methods and dispatch system
A type of function invoked depending on the class of one or more of its argument.

```{r example_glm}
#' print.name_of_the_class methods
methods(print)[92:96]
```
Dispatch:  
- examine the first argument and dispatches via a generic function depending on its class  
- dispatch by function naming convention that is describeed  
...
## S3: classes
Defind an S3 class `Passenger`.   

Loose definition. No valid check. No initialization. Single inheritance.  

```{r s3_class}
# Define S3 class
x <- list(name="Josie Andre", origin="SEA", destination="KCM")
class(x) <- "Passenger" # set valuse of the class attribute
...
## S4: Validation
Validation: slots must have the same length
...
2. Set validationg such that all slots have the same length
...
- _Advanced R_ (2nd ed.) by Hadley Wickham has 4 chapters:
